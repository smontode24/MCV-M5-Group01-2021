...
WEIGHTS = [3.466, 1.40, 1]
classes = ['car', 'person']

class FastRCNNOutputsFocalLoss(FastRCNNOutputs):
    """
    ...
    def softmax_cross_entropy_loss(self):
        ...
        inputs = self.pred_class_logits
        batch_size = self.gt_classes.size(0)
        targets = torch.zeros(batch_size, 2)
        targets[torch.arange(batch_size), self.gt_classes] = 1
        device = "cuda:"+str(self.pred_class_logits.get_device())
        targets = targets.to(device)

        weights = torch.tensor(WEIGHTS).float().to(device)
        p = torch.sigmoid(inputs)
        ce_loss = F.binary_cross_entropy_with_logits(
            inputs, targets, reduction="mean", weight=weights
        )
...
class FastRCNNOutputLayersFocalLoss(FastRCNNOutputLayers):
    """
    ...
    def losses(self, predictions, proposals):
        ...
        losses = FastRCNNOutputsFocalLoss(
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeadsFocalLoss(StandardROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    Each head independently processes the input features by each head's
...
def modify_head_focalloss_class(cfg):
    cfg["MODEL"]["ROI_HEADS"]["NAME"] = "StandardROIHeadsFocalLoss"
    return cfg